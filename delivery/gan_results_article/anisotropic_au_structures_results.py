...
from plotting_routines.plots import (gan_plotter, gan_prediction_plotter, gan_prediction_plot,
                                     gan_prediction_comparison, gan_big_prediction_plot, truncate_colormap)
...
fig_dir = "figures/anisotropic_au_structures/"
...
# Setup dataloader
root_dir = Path("../anisotropic_au_structures_train_val_test")
...
test_loader2 = DataLoader(test_dataset, batch_size=1000, pin_memory=True)

################ PLOTS FOR CNN STARTS HERE ##########################
# Load trained model
checkpoint = torch.load("cnn_results/last_epoch_effv2_cross_all_structures_lr00001_drop05.pth.tar", map_location=device)
...
filenames = ["fcgan_results/wgan_fc_cross_all_FF.pth.tar",
             "fcgan_results/wgan_fc_cross_all_TF.pth.tar",
             "fcgan_results/wgan_fc_cross_all_FT.pth.tar",
             "fcgan_results/wgan_fc_cross_all_TT.pth.tar"
             ]
legends = ["FCGAN",
           "FCGAN + LP",
           "FCGAN + Embed.",
...
for i, f in enumerate(filenames):
    ...
    append_to_data_plot(ax[0], rce[:, 0], rce[:, 3], legends[i], colors[i], markers[i])

# Plot cnn error (|Pred. Real - Pred. Fake|) from CNN network
for i, f in enumerate(filenames):
    ...
    append_to_data_plot(ax[1], fwde[:, 0], fwde[:, 4], legends[i], colors[i], markers[i])
...
generator2 = Generator(INPUT_CHANNELS, target_channels=variable.size(), y_dim=Y_DIM, z_dim=Z_DIM,
                      image_size=IMAGE_SIZE).to(device)
generator3 = Generator(INPUT_CHANNELS, target_channels=variable.size(), y_dim=Y_DIM, z_dim=Z_DIM,
                      image_size=IMAGE_SIZE).to(device)
generator.eval()
generator2.eval()
generator3.eval()
...
########## DCGAN + LP + Embed on full dataset ############
checkpoint = torch.load("dcgan_results/wgan_dc_cross_all_TT.pth.tar", map_location=device)
dc_generator.load_state_dict(checkpoint['generator_state_dict'])

fig = gan_plotter(dc_generator, test_loader, "DCGAN + LP + Embed")
fig.savefig(fig_dir + "dcgan_TT_example_predictions.png", format='png', dpi=100, bbox_inches='tight')


######### Test set predictions, FCGAN, FCGAN + LP, FCGAN + LP + Embed. ########
checkpoint = torch.load("fcgan_results/wgan_fc_cross_all_FF.pth.tar", map_location=device)
checkpoint2 = torch.load("fcgan_results/wgan_fc_cross_all_TF.pth.tar", map_location=device)
checkpoint3 = torch.load("fcgan_results/wgan_fc_cross_all_TT.pth.tar", map_location=device)
generator.load_state_dict(checkpoint['generator_state_dict'])
generator2.load_state_dict(checkpoint2['generator_state_dict'])
generator3.load_state_dict(checkpoint3['generator_state_dict'])

# 4, 33, 155, 406, 256, 301
fig = gan_prediction_comparison(generator, generator2, generator3, test_loader2, [110, 4, 301, 256, 155, 406], Z_DIM,
                                ["FCGAN", "FCGAN + LP", "FCGAN + \nLP + Embed."])
fig.savefig(fig_dir + "fcgan_prediction_comparison.png", format='png', dpi=150, bbox_inches='tight')
fig.savefig(fig_dir + "fcgan_prediction_comparison.svg", format='svg', dpi=100, bbox_inches='tight')

indices = [3, 215, 11, 31, 68, 90,
           305, 94, 95, 97, 102, 112,
           159, 188, 5, 222, 254, 85]
# 3, 5, 11, 31, 68, 90, 85, 94, 95, 97, 102, 112, 159, 188, 215, 222, 254, 305
fig, fig2 = gan_big_prediction_plot(generator3, forward_net, test_loader, training_dataset,
                              indices, Z_DIM, Y_DIM, "FCGAN + LP + Embed")
fig.savefig(fig_dir + "fcgan_TT_example_predictions.png", format='png', dpi=150, bbox_inches='tight')
fig.savefig(fig_dir + "fcgan_TT_example_predictions.svg", format='svg', dpi=100, bbox_inches='tight')
fig2.savefig(fig_dir + "fcgan_TT_stochastic_predictions_single_label.png", format='png', dpi=100,
                    bbox_inches='tight')
