...
from src.utils import add_filename_suffix, clear_old_files
...
class BaseGanTrainer:
    def __init__(
        ...
        if self.save_model_filename:
            ...
            if index_of_last_slash == -1:
                ...
                self.save_image_dir = (
                    save_model_filename[:index_of_last_slash]
    ...
    def get_image_grids(self, n_images=N_IMAGES):
        ...
        with torch.no_grad():
            ...
            train_real, train_labels = (
                train_real.to(self.device),
            ...
            train_fake = self.generator(
                fixed_noise[:n_images].to(self.device), train_labels[:n_images]
            )
            val_fake = self.generator(
                fixed_noise[:n_images].to(self.device), val_labels[:n_images]
            )
    ...
    def write_reconstruction_error_to_tensorboard(
        ...
        if self.write_to_tensorboard:
            ...
            self.writer.add_scalar(
                "Structural_Reconstruction_error/Training", train_struct_rce, epoch
            )
            self.writer.add_scalar(
    ...
    def save_checkpoint(self, suffix=None, clear_old=False):
        ...
        if clear_old:
            clear_old_files(self.save_model_filename)
    ...
    def save_images(self, epoch, images):
        ...
        data = (
            torch.stack((train_real, train_fake, val_real, val_fake))
            .to(torch.device("cpu"))
            .numpy()
        )
