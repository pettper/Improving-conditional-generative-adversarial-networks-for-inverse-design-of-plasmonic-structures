...
class DCGANCritic(nn.Module):
    ...
    def __init__(self, inp_channels, target_channels=2, y_dim=81, proj_dim=50, image_size=128, features=4, lp=True,
        ...
        if self.image_size == 64:
            self.down_sampling_block = DCGANDiscriminator64(inp_channels, proj_dim, features=features)
        elif self.image_size == 128:
            self.down_sampling_block = DCGANDiscriminator128(inp_channels, proj_dim, features=features)
        else:
