...
class DCGANDiscriminator128(nn.Module):
    ...
    def __init__(self, inp_channels, y_dim, features=4, spectral_normalization=False):
        ...
        self.down_sampling_block = nn.Sequential(
            snorm(nn.Conv2d(inp_channels, features * 4, kernel_size=4, stride=2, padding=1, bias=False)),     # 64x64
            nn.LeakyReLU(SLOPE),
            snorm(nn.Conv2d(features * 4, features * 8, kernel_size=4, stride=2, padding=1, bias=False)),    # 32x32
            nn.InstanceNorm2d(features * 8, affine=True),
            nn.LeakyReLU(SLOPE),
            snorm(nn.Conv2d(features * 8, features * 16, kernel_size=4, stride=2, padding=1, bias=False)),    # 16x16
