...
class FusedSETransposedInvertedResidualBlock(nn.Module):
    ...
    def __init__(self, inp_channels, output_channels, expansion_ratio=6, reduction_ratio=4, kernel_size=4, stride=2,
        ...
        if stride == 1:
            self.conv_block = nn.Conv2d(inp_channels, hidden_dim, kernel_size=kernel_size, stride=stride,
                                        padding=padding, bias=bias)
        else:
            self.conv_block = nn.ConvTranspose2d(inp_channels, hidden_dim, kernel_size=kernel_size, stride=stride,
                               padding=padding, output_padding=stride - 1, bias=bias)
