...
class DimerDataset(Dataset):
    ...
    def __init__(self, root_dir, variable, transform=None, target_transform=None, inverse_transform=None, inverse_target_transform=None, number_of_samples=None):
        ...
        if self.variable == DimerVariable.ALL or self.variable == DimerVariable.CROSS_SECTIONS:
        ...
        for idx in range(N):
            ...
            # Construct sample
            # isAu channel
            self.image_data[idx, 0, :, :] = torch.Tensor(image_data.iloc[:, 2].values).reshape((1, 128, 128))
            # topylogy channel
            self.image_data[idx, 1, :, :] = torch.Tensor(image_data.iloc[:, 3].values).reshape((1, 128, 128))
            # output variable
            if self.variable == DimerVariable.ALL or self.variable == DimerVariable.CROSS_SECTIONS:
                self.output_data[idx, :] = torch.Tensor(output_data.iloc[:, self.variable.value].values).transpose(0, 1)
            else:
                self.output_data[idx, :] = torch.Tensor(output_data.iloc[:, self.variable.value].values)
        ...
        # Targets
        if self.variable == DimerVariable.CROSS_SECTIONS or self.variable == DimerVariable.ALL:
    ...
    def default_target_transform(self, output_data):
        if self.variable == DimerVariable.ALL or self.variable == DimerVariable.CROSS_SECTIONS:
            output_data[:, 0, :] = (output_data[:, 0, :] - self.sca_min) / (self.sca_max - self.sca_min)
            output_data[:, 1, :] = (output_data[:, 1, :] - self.abs_min) / (self.abs_max - self.abs_min)
        if self.variable == DimerVariable.ALL:
            output_data[:, 2, :] = (output_data[:, 2, :] - self.rot_min) / (self.rot_max - self.rot_min)
            output_data[:, 3, :] = (output_data[:, 3, :] - self.elip_min) / (self.elip_max - self.elip_min)
        else:
            if self.variable == DimerVariable.SCATTERING_CROSS_SECTION:
                output_data = (output_data - self.sca_min) / (self.sca_max - self.sca_min)
            elif self.variable == DimerVariable.ABSORPTION_CROSS_SECTION:
                output_data = (output_data - self.abs_min) / (self.abs_max - self.abs_min)
    ...
    def get_min_and_max_data(self):
        ...
        if self.variable == DimerVariable.ALL:
            return (self.shape_min, self.shape_max, self.top_min, self.top_max), (self.sca_min, self.sca_max,
                                                                                  self.abs_min, self.abs_max,
                                                                                  self.rot_min, self.rot_max,
                                                                                  self.elip_min, self.elip_max)
        elif self.variable == DimerVariable.CROSS_SECTIONS:
            return (self.shape_min, self.shape_max, self.top_min, self.top_max), (self.sca_min, self.sca_max,
                                                                                  self.abs_min, self.abs_max)
        else:
            return (self.shape_min, self.shape_max, self.top_min, self.top_max), (self.output_min, self.output_max)
