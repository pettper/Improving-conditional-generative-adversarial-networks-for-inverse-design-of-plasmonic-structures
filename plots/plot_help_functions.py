...
def gan_prediction_comparison(generator_dict, dataloader, indices, z_dim):
    ...
    with torch.no_grad():
        ...
        fig, ax = plt.subplots(number_of_generators+1, num_images, figsize=(12, 2*(number_of_generators + 1)))
        ...
        for i in range(num_images):
            im_plot = ax[0][i].imshow(real[i, 1, :, :], vmin=-1, vmax=1, cmap=cmap)   # To plot the original images
            plt.setp(ax[0][i], xticks=[], yticks=[]) # To remove the ticks from all images
        ...
        for j, k in enumerate(generator_dict.keys()):
            ...
            for i in range(num_images):
                ax[j+1][i].imshow(fake[i, 1, :, :], vmin=-1, vmax=1, cmap=cmap)   # To plot images from GAN-network
                plt.setp(ax[j+1][i], xticks=[], yticks=[]) # To remove the ticks from all images
...
def gan_big_prediction_plot(generator, forward_network, dataloader, inverse_target_transform,
                            indices, z_dim, network_label):
    generator.eval()
    forward_network.eval()
    with torch.no_grad():
        ...
        for i in range(rows // 3):
            for j in range(cols):
                im = i*cols + j
                im_plot = ax[i][j].imshow(torch.cat((real[im, 1, :, :], fake[im, 1, :, :]), dim=1), vmin=-1, vmax=1, cmap=cmap)   # Original images plot
                ax[i][j].axvline(128, color='black', linewidth=1)    # Split images with a vertical line
                # Scattering cross-section
                ax[(rows//3) + i][j].plot(lda, original_labels[im, 0, :], label="FEM", linewidth=lw, linestyle="solid")
                ax[(rows//3) + i][j].plot(lda, real_labels[im, 0, :], label="Pred. real", linewidth=lw, linestyle="dashdot")
                ax[(rows//3) + i][j].plot(lda, fake_labels[im, 0, :], label="Pred. " + network_label,
                                linewidth=lw, linestyle="dashed")
                # Absorption cross-section
                line1, = ax[2*(rows//3) + i][j].plot(lda, original_labels[im, 1, :], label="FEM", linewidth=lw, linestyle="solid")
                line2, = ax[2*(rows//3) + i][j].plot(lda, real_labels[im, 1, :], label="Pred. real", linewidth=lw, linestyle="dashdot")
                line3, = ax[2*(rows//3) + i][j].plot(lda, fake_labels[im, 1, :], label="Pred. " + network_label,
                                linewidth=lw, linestyle="dashed")
                plt.figlegend(handles=[line1, line2, line3], fontsize=FS+4, loc='lower right', ncol=3,
        ...
        for i in range(rows // 3):
            for j in range(cols):
                ...
                ax[i][j].annotate(annotations[i][j], fontsize=FS+4, xy=(0.08, 0.80), xycoords='axes fraction')
                ax[(rows//3) + i][j].annotate(annotations[i][j], fontsize=FS+4, xy=(0.08, 0.80), xycoords='axes fraction')
                ax[2*(rows//3) + i][j].annotate(annotations[i][j], fontsize=FS+4, xy=(0.08, 0.80), xycoords='axes fraction')
        ...
        plt.annotate("Sca. cross sec. [m^2]", (0.05, 0.365), xycoords="figure fraction", fontsize=FS+6, rotation=90)
        plt.annotate("Abs. cross sec. [m^2]", (0.05, 0.1), xycoords="figure fraction", fontsize=FS+6, rotation=90)
        plt.annotate("Wavelength [nm]", (0.403, 0.05), xycoords="figure fraction", fontsize=FS+6)
        plt.annotate("Original | " + network_label, (0.30, 0.86), xycoords="figure fraction", fontsize=FS+8, weight='bold')
