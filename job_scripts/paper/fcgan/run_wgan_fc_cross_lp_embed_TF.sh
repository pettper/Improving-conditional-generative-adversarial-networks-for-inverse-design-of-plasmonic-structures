...
lp=True			# Use label projection?
embed=False		# Use embedding network?
fn=saved_models/paper_results/cnn_results/last_epoch_effv2_cross_lr00001_drop05.pth.tar		# Feed forward network to use as metric
save_name=saved_models/wgan_fc_cross_cylinder_TF.pth.tar
load_name=saved_models/paper_results/fcgan_results/wgan_fc_cross_cylinder_TF.pth.tar
data_dir=data/dimer_cylinder_train_val_test/
python wgan_gp_main.py -a ${arch} -e ${epochs} -var ${var} -b ${bs} -lr ${lr} -lp ${lp} -em ${embed} -ffn ${fn} -lf ${load_name} -sf ${save_name} -ddir ${data_dir}
